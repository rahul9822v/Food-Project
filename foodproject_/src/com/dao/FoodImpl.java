...
public class FoodImpl implements FoodDao {
	...
	@Override
	public boolean delete_food_by_id(int f_id) {
		// TODO Auto-generated method stub
	...
	public food get_food_byid(int f_id) {
		...
		try {
			...
			if(rs.next())
			{
				f.setF_id(rs.getInt("f_id"));//if we dont known column name i sql so type column number
	...
	public List<food> get_all_food() {
		...
		try {
			...
			{
				food f=new food();
				f.setF_id(rs.getInt("f_id"));//if we dont known column name in sql so type column number
				f.setF_name(rs.getString("f_name"));
				f.setF_type(rs.getString("f_type"));
				f.setF_category(rs.getString("f_category"));
				f.setF_price(rs.getFloat("f_price"));
